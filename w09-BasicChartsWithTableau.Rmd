...
The Hawks data set was collected by students from Cornell College. It is built in in the R library {Stat2Data}. I also make a copy and posted at <https://raw.githubusercontent.com/pengdsci/sta553/main/Tableau/hawks.csv>. You can download it and save it to a folder in your machine.
...
Tableau uses manual driven approach to load data in certain formats. The following figure shows the types of external data that be connected to Tableau. There are some built-in data sets available in Tableau for practice purpose.
...
The Hawks data is in CSV format, we choose `text file` to connect to the data set. After the data being connected, we will see the following Data Source page with the brief information of the data set.
...
Statistical charts are created on different sheets. We can add more sheets as needed and change default sheets name to a meaningful name.
...
**Step 1**: repeat the two steps in barchart.

**Step 2**: Click `Show Me` on the topright of the UI and select `piechart icon`.
...
**Step 3**: Click `color` icon in Marks panel to adjust color and border of the histogram.
...
**Step 2**: Click `Show Me` on the topright of the UI and select `line plot` icon.

**Step 3**: right click `Species` and `Sex` and send them to the **Filter** panel.

**Step 4**: Choose appropriate display form of the filter (see the right panel of the following screenshot)
...
Let's explore the association between the lengths of wing and tail of hawks across the species. The following steps create a simple scatter plot in Tableau. 

**Step 1**: drag the two numerical variables to column and row fields.

**Step 2**: Change the two aggregated variable (by default) to `dimension` (see the left hand side screenshot).

**Step 3**: Color code the species (drag `spcies` to the **color** mark).

**Step 4**: Choose the categorical variables to define filters to explore the association of subset of the data (partial association) using drop-down menu, radio button, or slider, etc.
...
A bubble chart is modified from a regular scatter plot. We next use the  above scatter plot as a base plot and make the point size be proportional to the value of variable `wing`.

**Step 1**: create a basic scatter plot (following steps 1-4 in the previous section of scatter plot).
...
**Step 4**: convert `year` to a string variable and add `sex` and `year` to filter.
...
The following steps will create a map to view spatial distribution of properties in the bay area.
...
**Step 4**. click the `Color` shelf in the Marks field, change default color to an appropriate color.

**Step 5**. Choose appropriate color.

**Step 6** select appropriate variable to determine the point size .

**Step 7** drag variable you want to display in the hover text.

The following screenshot show the above steps.
...
Let's use the POC (US gas station data) as example to how deal with many data points. The data can be found at: <https://github.com/pengdsci/datasets/raw/main/POC.csv>. We first download this data file and save it into a local folder and then connect Tableau to this data.
...
**Step 3**. In drop-down menu of the Marks field, select `density`.

**Step 4**. Go to the top menu bar, click `Map` to select a background map.

**Step 5**. click the `Color` shelf in the Marks field, change default color to an appropriate color.
...
# Basic Charts with Derived Variables 

Tableau has a lot built-in function that can be used to defined derived variables. This section use several examples to illustrate how to use some of the commonly used functions for creating statistical graphics. The complete list of these built-in functions can be found at <https://help.tableau.com/current/pro/desktop/en-us/functions_all_alphabetical.htm>


## Histogram with A Normal Density Curve

We use variable `weight` as an example to create a histogram and then place a normal density on the top of it.


<iframe><src = "https://public.tableau.com/views/US-States-Facts/Sheet1?:embed=true"></iframe>


<iframe align = "center" width = "800" height = "600" src="https://public.tableau.com/views/US-States-Facts/Sheet1?:language=en-US&:display_count=n&:origin=viz_share_link"/>
